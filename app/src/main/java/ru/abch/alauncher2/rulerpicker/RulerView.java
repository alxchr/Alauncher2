...
package ru.abch.alauncher2.rulerpicker;
 ...
 * ABCh 07 Feb 2020
 * Added float_scale attribute for Ruler
 */
...
final class RulerView extends View {
    ...
    private void parseAttr(@Nullable AttributeSet attributeSet) {
        if (attributeSet != null) {
            ...
            try { //Parse params
                ...
                if (a.hasValue(R.styleable.RulerValuePicker_float_scale)) {
                    setFloatScale(a.getFloat(R.styleable.RulerValuePicker_float_scale, 1f));
                }
            } finally {
                a.recycle();
            }
    ...
    private void drawValueText(@NonNull final Canvas canvas,
        ...
        String sValue;
        if (mFloatScale == 1f) {
            sValue = String.valueOf(value + mMinValue);
        } else {
            float fValue = (value + mMinValue) * mFloatScale;
